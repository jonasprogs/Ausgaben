...
// Passcode (simple hash in meta)
async function getPassHash(){ const m = await get('meta','passhash'); return m?.value || null; }
async function setPassHash(pin){
  const hash = await sha256(pin);
  return put('meta',{key:'passhash', value:hash});
}
...
document.getElementById('setPinBtn').addEventListener('click', async ()=>{
  ...
  if(!pin || pin.length<4 || pin.length>8){ alert('Bitte 4–8-stelligen Passcode wählen.'); return; }
  await setPassHash(pin);
  alert('Passcode gespeichert.');
  document.getElementById('newPin').value='';
});
async function requireUnlock(){
  const hash = await getPassHash();
  if(!hash){ lockEl.style.display='none'; return; }
...
// WebAuthn (Face ID / Touch ID) minimal client-side (safe wrappers)
(function(){

  function safeSupportedCheck(){
    try{
  ...
  async function webauthnSupported(){
    if(!safeSupportedCheck()) return false;
    try{
      return await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
    }catch(e){ return false; }
  }

  async function getCredId(){ const m = await getMeta('webauthn_cred_id'); return m?.value || null; }
  async function setCredId(id){ return setMeta('webauthn_cred_id', id); }
  function b64ToBuf(b64){ try{ return Uint8Array.from(atob(b64), c=>c.charCodeAt(0)); }catch(e){ return new Uint8Array(); } }

  async function webauthnRegister(){
    const sup = await webauthnSupported();
    if(!sup){ alert('Face ID/Touch ID nicht verfügbar (HTTPS & Geräteunterstützung erforderlich).'); return; }
    try{
      const challenge = crypto.getRandomValues(new Uint8Array(32));
      const userId = crypto.getRandomValues(new Uint8Array(16));
      const pubKey = {
        challenge,
        rp: { name: 'Ausgaben' }, // rp.id weglassen für localhost/Custom Domains
      ...
      if(!cred){ alert('Registrierung abgebrochen.'); return; }
      const idB64 = btoa(String.fromCharCode(...new Uint8Array(cred.rawId)));
      await setCredId(idB64);
      alert('Face ID aktiviert.');
    }catch(err){
      console.warn('WebAuthn register error', err);
      alert('Face ID konnte nicht aktiviert werden.');
    }
  }

  async function webauthnAuth(){
    const sup = await webauthnSupported();
    if(!sup){ alert('Face ID/Touch ID nicht verfügbar.'); return; }
    const credId = await getCredId();
    if(!credId){ alert('Face ID ist noch nicht aktiviert.'); return; }
    try{
      const challenge = crypto.getRandomValues(new Uint8Array(32));
      const allow = [{ id: b64ToBuf(credId), type: 'public-key', transports: ['internal'] }];
      const pubKey = { challenge, allowCredentials: allow, userVerification: 'required', timeout: 60000 };
      const assertion = await navigator.credentials.get({ publicKey: pubKey });
      if(assertion){
        document.getElementById('lock').style.display='none';
        document.getElementById('lockPin').value='';
      }else{
        alert('Authentifizierung abgebrochen.');
      }
    }catch(err){
  ...
  window.__webauthn = { register: webauthnRegister, auth: webauthnAuth, supported: webauthnSupported };

  // Wire buttons only if present
  window.addEventListener('DOMContentLoaded', async ()=>{
    const regBtn = document.getElementById('webauthnRegister');
    const authBtn = document.getElementById('webauthnAuth');
    if(!regBtn || !authBtn) return;
    regBtn.addEventListener('click', ()=> window.__webauthn.register());
    authBtn.addEventListener('click', ()=> window.__webauthn.auth());
    const sup = await webauthnSupported();
    if(!sup){ regBtn.disabled = true; authBtn.disabled = true; }
  });
