...
function parseQuick(input){
  ...
  if(/\blebensmittel\b/i.test(text)){
    ...
    // if last token equals any known category name in DB, will resolve later in addExpense
    const tokens = text.split(/\s+/).filter(Boolean);
    if(tokens.length){
      const last = tokens[tokens.length-1].toLowerCase();
      // treat as category if short and alphabetic
      if(/^[a-zäöüß]+$/i.test(last)) {
        category = last;
        tokens.pop();
        text = tokens.join(' ');
      }
...
document.getElementById('parseBtn').addEventListener('click', async ()=>{
  ...
  const catId = await ensureCategory(p.category || 'lebensmittel');
