...
// WebAuthn (Face ID / Touch ID) minimal client-side
async function webauthnSupported(){
  return !!(window.PublicKeyCredential && navigator.credentials && typeof PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable === 'function' && await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable());
}
async function getCredId(){
  const m = await getMeta('webauthn_cred_id'); return m?.value || null;
}
async function setCredId(id){
  return setMeta('webauthn_cred_id', id);
}
...
async function webauthnRegister(){
  if(!(await webauthnSupported())){ alert('WebAuthn/Face ID wird auf diesem Gerät/Browser nicht unterstützt.'); return; }
  const credId = await getCredId();
  if(credId){ if(!confirm('Face ID ist bereits aktiviert. Erneut registrieren?')) return; }
  const challenge = crypto.getRandomValues(new Uint8Array(32));
  const userId = crypto.getRandomValues(new Uint8Array(16));
  const pubKey = {
    challenge,
    rp: { name: 'Ausgaben', id: location.hostname || undefined },
  ...
  // store credential id (base64) locally
...
async function webauthnAuth(){
  ...
  if(assertion){
    // Success - treat as authenticated
...
// Wire buttons
(async ()=>{
  const sup = await webauthnSupported().catch(()=>false);
  const regBtn = document.getElementById('webauthnRegister');
  const authBtn = document.getElementById('webauthnAuth');
  if(regBtn) regBtn.addEventListener('click', ()=> webauthnRegister().catch(err=> alert('Fehler: '+err)));
  if(authBtn) authBtn.addEventListener('click', ()=> webauthnAuth().catch(err=> alert('Fehler: '+err)));
  if(!sup){
