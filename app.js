...
// Quick parser: amount, date keywords, name (rest). Default category: Lebensmittel.
...
function parseQuick(input){
    ...
    for(const tok of tokens){
      ...
      if(DATE_WORDS.includes(t)){
        ...
        if(['gestern'].includes(t)) d.setDate(d.getDate()-1);
          ...
          const map = {'mo':1,'montag':1,'di':2,'dienstag':2,'mi':3,'mittwoch':3,'do':4,'donnerstag':4,'fr':5,'freitag':5,'sa':6,'samstag':6,'so':0,'sonntag':0};
          const target = map[t];
          const cur = d.getDay();
          let diff = (cur - target);
          if(diff<=0) diff += 7;
  ...
  // category: only if explicitly says lebensmittel, else default lebensmittel
  let category = null;
...
document.getElementById('parseBtn').addEventListener('click', async ()=>{
  ...
  const catId = await ensureCategory('lebensmittel');
